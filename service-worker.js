/* タイ語単語帳 PWA Service Worker
   ...
   語彙データを更新して index_v6.1.html を差し替えたら、必ずここも上げること。 */
const CACHE_VERSION = 'thai-vocab-v6.1.0';
const CORE_ASSETS = [
  './',
  './index_v6.1.html',
  './manifest.json',
  ...
  event.waitUntil(
    caches.open(CACHE_VERSION)
      .then((cache) => cache.addAll(CORE_ASSETS).catch(() => {
        ...
        return cache.addAll(['./index_v6.1.html']);
      }))
  ...
  if (isHTML) {
    event.respondWith(
      fetch(req)
        ...
        .catch(() => caches.match(req).then((r) => r || caches.match('./index_v6.1.html')))
    );
    return;
  }
